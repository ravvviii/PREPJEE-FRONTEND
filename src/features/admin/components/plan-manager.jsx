...
const EMPTY = { name: '', amount: 100, currency: 'INR', durationDays: 30, isActive: true, isDefault: false };
...
export function PlanManager() {
  ...
  const save = useMutation({
    mutationFn: () => {
      const fields = { ...form, amount: Number(form.amount), durationDays: Number(form.durationDays) };
      if (!editing?.id) {
    ...
    <>
      ...
      <Dialog open={Boolean(editing)} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent>
          ...
          <DialogFooter><Button onClick={() => save.mutate()} disabled={!form.name || Number(form.amount) < 100}>Save</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </>
  );
